...
const EventModal: React.FC<EventModalProps> = ({ date, shift, event, existingBirthday, onSave, onDelete, onSaveBirthday, onDeleteBirthday, onClose }) => {
  ...
  const handleSave = () => {
    ...
    // Save regular event data
    onSave(dateKey, { note, hasVacation, colleagues: finalColleagues, isAfz });
  ...
  const hasContent = event && (event.note.trim() || event.hasVacation || event.isAfz);
